...
assert.equal(pipe.domains_are_doors, false);
assert.equal(pipe.domain_doors.domains_are_doors, false);
assert.equal(pipe.domain_doors.role, "inspection_frame");
assert.match(pipe.note, /inspection frame/);
assert.match(pipe.note, /domains_are_doors:false/);
assert.equal(pipe.note.includes("Domain Door"), false);
assert.match(PIPELINE_PATH, /inspection frame/);
assert.equal(PIPELINE_PATH.includes("Domain Door"), false);
...
assert.ok(pipe.placements.some((p) => p.slug === "azcoherence" && p.placement === "scoring-review" && p.bucket === "plain"));
...
assert.match(SKILL_MD, /fraggate_describe/);
assert.match(SKILL_MD, /AZ-CLCE/);
assert.match(SKILL_MD, /AKM-TRIAD/);
assert.match(SKILL_MD, /GET \/v1\/software/);
assert.match(SKILL_MD, /plain A–Z/);
...
const cited = await dispatch("pipeline_arch", {});
const citeFields = Object.fromEntries((cited.display.fields || []).map((f) => [f.label, f.value]));
assert.equal(citeFields["4dmap"], FOURDMAP_FRAME);
assert.equal(cited.domains_are_doors, false);
...
assert.match(html, /4DMap inspection frame/);
assert.match(html, /domains_are_doors:false/);
assert.equal(html.includes("Domain Door"), false);
...
assert.match(spec.info.description, /inspection frame/);
assert.match(spec.info.description, /domains_are_doors:false/);
assert.equal(spec.info.description.includes("Domain Door"), false);
