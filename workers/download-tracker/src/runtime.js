...
function toolDefs() {
  const desc = {
    ...
    pipeline_arch: "Cite MASTER-ARCHITECTURE-2.0 hop order. FragGate is THE SINGLE DOOR. Runtime owns fabric hops. 33/11 domain layer. No LambGate.",
...
<p>LOCKED suite pipeline (MASTER-ARCHITECTURE-2.0): FragGate is THE SINGLE DOOR. Human → AZInterface → PUBLIC/UI/AGENT/API → FragGate → Lamb Lens → SweepGate → Sentinel → Provenance/Input Packet → ChainLock-IN → DecisionGATE → AZPIPE → Internal Domain Layer (33/11; 4DMap) → optional ASE → RoseClock → TemporalLock → ChainLock-OUT → ForgeReceipts → Return. Runtime owns fabric hops. SUITE-PIPE-1.6.15 kept. No LambGate. Local <code>/v1/pipeline_arch</code>.</p>
