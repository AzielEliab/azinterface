...
function openapiSpec(origin) {
  ...
  paths["/v1/mesh"] = {
    get: {
      ...
      summary: "PROXY to aziel-runtime GET /v1/mesh. QNM-BUILD-1.0 suite rollup. SPLIT THE WIRES + COLD-COPY SURVIVAL + REHEAL refuse cite. Default OFF. Never enables. Not a local op. Not a Node Gate. Not AnonBroadcast.",
  ...
  paths["/v1/mesh/heartbeat"] = {
    post: {
      ...
      summary: "PROXY to aziel-runtime POST /v1/mesh/heartbeat. Presence refresh. Heartbeat loss≠poison≠apply last packet. No auto-heal.",
      requestBody: { content: { "application/json": { schema: { type: "object" } } } },
...
<p>Suite mesh <code>/v1/mesh/*</code> PROXIES to aziel-runtime (QNM-BUILD-1.0 Live Nodes read-only; GET never enables). SPLIT THE WIRES + COLD-COPY SURVIVAL. REHEAL refuse. QNS-CD-1.0 vias run in local <code>qnsd</code> (127.0.0.1). Interface holds pair memorial — not a Softwares-tab QNS product. AIH-WP-1.3 spiderweb is local <code>qnm-node/</code> — not a public Node Gate. Anon-broadcast is not a publish path.</p>
